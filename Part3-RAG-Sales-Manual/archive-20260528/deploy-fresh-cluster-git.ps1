# Fresh OpenShift Cluster Deployment Script (PowerShell) - Git-Based Builds
# Deploys complete RAG Sales Manual demo to a new IBM Power cluster
# Uses Git repository for builds - no local source code upload needed

Write-Host "=========================================="
Write-Host "IBM Power RAG Sales Manual"
Write-Host "Fresh Cluster Deployment (Git-Based)"
Write-Host "=========================================="
Write-Host ""
...
$GIT_REPO = "https://github.com/DSpurway/IBM-Power-RAG-Demos"
...
Write-Host ""
Write-Host "Build Method: Direct from Git (no local upload)"
Write-Host ""
...
# Create new-app from Git (builds and deploys in one command)
Write-Host "Building from Git repository..."
oc new-app $GIT_REPO `
  --context-dir=Part3-RAG-Sales-Manual/granite-service `
  --name=granite-service `
  --strategy=docker

Write-Host "Waiting for build to complete..."
oc logs -f bc/granite-service

Write-Host "Exposing Granite service..."
oc expose svc/granite-service
...
Write-Host "Waiting for build to complete..."
oc logs -f bc/llama-service

Write-Host "Exposing TinyLlama service..."
oc expose svc/llama-service
...
Write-Host "Exposing RAG backend..."
...
if (-not $OPENSEARCH_URL) { $OPENSEARCH_URL = "N/A (internal only)" }
...
Write-Host "  OpenSearch:    $OPENSEARCH_URL"
...
Write-Host ""
Write-Host "Benefits of Git-based deployment:"
Write-Host "  ✅ No local source code upload needed"
Write-Host "  ✅ Faster deployment (no file transfer)"
Write-Host "  ✅ Always uses latest code from repository"
Write-Host "  ✅ Minimal local files required"
